...
// Same-dimension conversions are exact (fixed physical ratios).
// Cross-dimension (mass <-> volume) requires a density in g/ml.
 ...
 * @returns { ok:true, value, bridged } | { ok:false, reason }
 */
export function convert(qty, fromUnit, toUnit, densityGml) {
  ...
  const isMassVol =
    (f.dim === 'mass' && t.dim === 'volume') ||
    (f.dim === 'volume' && t.dim === 'mass')

  if (isMassVol && densityGml) {
    let grams
    if (f.dim === 'mass') grams = qty * f.toBase
    else grams = (qty * f.toBase) * densityGml
    if (t.dim === 'mass') return { ok: true, value: grams / t.toBase, bridged: true }
    const ml = grams / densityGml
    return { ok: true, value: ml / t.toBase, bridged: true }
  }
